...
def gapminder_part():
    data = pd.read_csv(r'C:\Users\USER\Documents\Courses\Coursera\dai\datasets\gapminder\gapminder.csv', low_memory=False)
    ...
    # scat1 = seaborn.regplot(x="urbanrate", y="internetuserate", data=data)
...
def test_plotting():
    x = list(range(1, 9))
    y = np.random.rand(1, 8)[0]  # Get 8 random floats from range 0,1
    print(x)
    print(y)
    plt.plot(x, y)
    plt.show()
